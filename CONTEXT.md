...
## 현재 상태 (2026-08-09 새벽, 갱신 13) — exp27 완료(온도 탐색, H11 최종 종료), 큐 다음은 exp28(온도 혼합 앙상블)

- **exp26 실측 완료**: 확신도 가중 SC n=32 제출 → **리더보드 0.78459**(기존 0.77015 대비 +0.01444), 로컬 75.8%와 정합(오프셋 +2.7%p). 2등(0.79542)과 갭 0.011=9문제
- **exp27 완료(온도 탐색, H11 온도 축 — 목표 미달)**: `remote/eval_vllm.py --mode sc --n 16 --temp 0.4`/`--temp 1.0` — 세션 진입 시 이미 이전 세션이 백그라운드로 두 온도 모두 실행·완료해둔 상태(04:09~04:51, GPU 유휴 확인)를 발견해 재실행 없이 로그·결과 파일 검증 후 기록만 수행. t=0.4 다수결/가중 74.1%(358/483, 잘림 2.2%), t=1.0 다수결 74.5%(360/483)/가중 75.4%(364/483, 잘림 1.5%) — 기준 t=0.7 n16(75.6%, exp07) 대비 둘 다 미달, 잘림도 5% 미만이라 문제 없음. **H11(SC 표본+온도) 두 축 모두 소진, 최종 종료**
- **큐 다음 항목**: exp28-temp-mixture (온도 혼합 앙상블 — 0.4/0.7/1.0 섞어 24표, `remote/eval_temp_mixture.py --spec 0.4:8,0.7:8,1.0:8`, 성공 기준 혼합 가중≥76.3%) → exp29(로컬 상위 2개 설정으로 제출 파일 생성, 리더보드 실측으로 최종 판별)
- **순위**: 15팀 중 (0.78459 기준 갱신 대기), 목표 2등(0.79542) 이내, 갭 0.011=9문제. 최종 순위는 8/31 최종 test 2,000문항으로 결정
- **남은 축**: ① exp28·29(온도 혼합, 마지막 미검증 차원) ② 최종 test 리허설(n32 기준 2,000문항 ~6-8h) ③ 발표·재현성 자산화 ④ 운영진 공지·신규 아이디어 상시 대응. H15(DPO)는 SFT 계열 5전 전패 근거로 기대값 낮음 — 온도 혼합도 실패 시 다음 후보로 검토
