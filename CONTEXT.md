...
## 현재 상태 (2026-08-10 밤, 갱신 23) — exp44 완료(trim_lowconf 정밀 탐색, 성공기준 미달·집계 트랙 종료), exp45 skip, 큐 다음은 exp37(쌍대 비교 토너먼트)

- **exp44 완료(trim_lowconf 정밀 탐색 — GPU 불필요, 성공 기준(+8문제) 미달)**: `remote/sweep_aggregation.py`를 확장(커밋) — trim 비율 스윕(10~50%)·trim+가중 결합·trunc제외+trim 조합·길이기반 trim·trim+최소득표 폴백 27개 변형을 n={16,32,64}로 일괄 비교(수 초). 최고치는 n=32 76.4%(369/483, +5, 7전략 동률)로 **exp31b와 완전히 동일** — 정교화해도 같은 상한(pass@32=87.8%의 같은 부분집합)에 수렴. n=16/64도 각 +3/+2로 소폭 개선했으나 n=32를 못 넘음. 전 변형 성공 기준(+8) 미달로 **집계 규칙 탐색 트랙 최종 종료**(exp31b의 잠정 상한이 확정 상한). results/sweep44.log(gitignore *.log라 미커밋)
- **exp45 skip**: exp44가 기존(exp31b) 대비 갱신된 최고 전략을 내지 못해(동률) 실행 조건 미충족. 기존 submission_agg.csv(trim25%/n32)는 여전히 제출 보류 중 — 다음 가용일에 제출 필요
- **큐 다음 항목**: exp37-pairwise-tournament(검증자 대안 — 쌍대 비교로 접전 문제만 재표결) → exp41(분산 축소 스택) → exp38(제출 다양성 유지, 하루 3건 이내) → exp39(최종 파이프라인 동결 문서) → exp40(idle guard, 체크 금지) → exp42(BF+SC 결합) → exp43(발표 자산화)
