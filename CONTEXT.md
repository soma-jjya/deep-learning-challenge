...
## 현재 상태 (2026-08-11, 갱신 31) — 재개 사이클 종료, 큐 0건 ⚠️ 인스턴스 과금 중

**exp46·37b·47·48 전부 완료. 미완료 큐 0건.** 최종 스택 **변경 없음**(무학습 베이스 + 확신도 가중 SC n=32, seed 42) — 채택 기준을 넘은 실험이 없다.

| 실험 | 결과 | 판정 |
|---|---|---|
| exp46 스택 스모크 | greedy 69.0%(58문항) | ✅ 통과 — DPO용 패키지 추가가 vLLM 생성 경로를 깨지 않음 |
| exp37b 쌍대 비교 | 76.0% (+1문항, +0.21%p) | ❌ H21 최종 기각 — **버그 수정 후 처음으로 실제 검증됨**(0→57문항) |
| exp47 DPO | greedy 70.6% / SC 75.2~76.6% | ❌ 성공 기준 미달, 효과가 노이즈에 묻힘 |
| exp48 시드 대응 비교 | 차이 +4 / +1 / **−7**문항 | ❌ **노이즈 확정 → H15 최종 기각, 학습 축 6전 종료** |

### 이 사이클에서 얻은 것 (수치보다 중요)
1. **exp37 기각은 무효였다** — 실패가 아니라 코드 버그(`logprobs=0` 누락)로 판정 로직이 한 번도 호출되지 않았던 것. 수정 후 재검증해 정당하게 기각. *"실패했다"와 "실행되지 않았다"를 구분하지 않으면 가설이 아니라 버그를 기각하게 된다*
2. **"다양성 붕괴" 진단을 우리가 반박했다** — exp09b 이후 유지해온 설명은 정확도 하락폭에서의 역추론이었다. 직접 재보니 DPO는 pass@32 87.8→88.4%, 고유답 5.06→5.12, 1위 득표율 77.8→77.5%로 **예측과 전부 반대**. 더 맞는 설명은 **"이 모델은 풀이의 정오를 인식하지 못한다"** (exp18c 절대채점 +0.5%p / exp37b 상대비교 +0.21%p / exp47 DPO 보상정확도 0.60 — 세 갈래 동일 결론)
3. **선택 여지의 순위별 분해** — 12%p 중 상위2후보 완벽선택은 +4.14%p, 상위3은 +7.25%p. **2등에 필요한 +1.5%p는 상위2 헤드룸의 35%면 충분**했다. 필요한 건 신탁이 아니라 1·2위를 가릴 심판 하나였다
4. **로컬 시드 노이즈 실측** — 베이스만으로도 시드에 따라 75.4~76.4%(1.0%p 폭). 초기 채택한 ±1%p 원칙이 실측 검증됨. 시드 42만 봤다면 DPO를 +0.83%p 개선으로 오판해 최종 스택에 무의미한 어댑터를 넣었을 것
5. **SSH 간헐 장애의 진짜 원인** — 네트워크 차단이 아니라 **ISP 공인 IP 로테이션**(한 세션 내 211.235.66.89 → 58.231.140.230). `aws/ensure-ssh.ps1`로 해결(멱등 재등록, `-Prune` 정리). **8/31에 SSH가 안 되면 이것부터 실행**

### ⚠️ 즉시 판단 필요
- **인스턴스가 켜져 있고 온디맨드 $1.21/h로 과금 중이며 큐는 비어 있다.** 추가 계획이 없으면 `powershell -File aws\stop.ps1`
- 남은 유일한 방향은 **상용 API로 더 강한 심판/교사 데이터 구축**(규칙 5.3 허용, `prd.md:30`). Sonnet 5 배치 약 $31~60 예상. 사용자 판단 대기

### venv 변경 이력 (재현 환경 제출 시 반영 필요)
TRL 0.24의 `DPOTrainer`가 `mergekit`을 강제 임포트하고, mergekit은 transformers 5.x에서 삭제된 `TRANSFORMERS_CACHE`를 참조한다.
- **금지**: `uv pip install mergekit` 그대로 실행 → transformers 5.14.1→5.12.1 다운그레이드로 **vLLM(=8/31 최종 파이프라인)이 위험**. 실제로 한 번 발생시켰고 즉시 원복
- **한 조치**: `--no-deps`로만 설치(mergekit, llm-blender, dataclasses-json, marshmallow, typing-inspect, mypy-extensions, weave, gql, backoff, graphql-core, tenacity, diskcache) + `train_dpo.py` 내부 shim 2종(`TRANSFORMERS_CACHE`, `warnings_issued`). **transformers 5.14.1 / vLLM 0.26.0 유지 및 생성 정상 동작(exp46) 확인 완료**

## 이전 상태 (2026-08-11, 갱신 30) — exp37b 정정판 완료(H21 최종 기각), 큐 다음은 exp47(DPO)

**exp37b 완료(쌍대 비교 토너먼트 정정판, H21 최종 기각)**: `remote/eval_pairwise_tournament.py --margin 3 --gen-n 32 --votes 5` — logprobs=0 버그 수정 확인(대표 풀이 양쪽 확보 **57/65문항**, 1차 무효 시도는 0건이었음). tournament 367/483(76.0%) vs baseline 366/483(75.8%), delta **+1문항(+0.21%p)** — 성공 기준(+5문항) 크게 미달. 이번엔 실제로 검증됐으므로(미검증이 아닌 확정 기각) **H21 최종 기각**. 세션 진입 시 이전 세션이 이미 정정판을 실행·완료해둔 결과(eval37b_pairwise.log 04:29 UTC 종료, GPU 유휴 확인)를 검증 후 기록만 대행(재실행 없음)

### 진행 중인 큐
1. ~~`exp46-stack-smoke`~~ — 완료(통과)
2. ~~`exp37b-pairwise-fix`~~ — 완료(H21 최종 기각, 위 참고)
3. `exp47-dpo` — **다음 실행 대상**. H15. `data/verifier.jsonl`(24k) 재활용, 추가 생성 0. **평가는 반드시 SC n=32로** (exp09b: greedy가 올라도 SC가 내려가면 최종 스택에선 손해)
...
### 보류 카드 (사용자 판단, 2026-08-11 — 변동 없음)
- 상용 API 증류(규칙 5.3 허용, `prd.md:30`) — 유일하게 천장이 높은 카드지만 이번 사이클은 제외. 되살릴 경우 예상 비용 Sonnet 5 배치 약 $31~60
- spot 인스턴스 전환(g5.2xlarge $0.44/h vs 온디맨드 $1.21/h) — 현재 온디맨드 유지. **비용이 계속 발생하므로 실험 종료 시 반드시 정지할 것**

## 이전 상태 (2026-08-11, 갱신 29) — ▶️ 탐색 재개, 인스턴스 기동 (exp46/37b/47)
...
- **큐**: exp46(스택 스모크) → exp37b(쌍대 비교 정정판) → exp47(DPO) — exp46·exp37b는 갱신 30에서 완료 확인
...
### 확정 사항
- **최종 스택**: 무학습 베이스 + 확신도 가중 SC n=32 (temp 0.7 / top_p 0.8 / seed 42). 근거는 `docs/final-pipeline.md` — LB에서 4회 관측되며 0.780 근방 수렴, 다른 모든 변형보다 같거나 높음
- **실행 절차**: `docs/final-pipeline.md` 하나만 따르면 됨 (재개 가능한 dump 방식 필수 — make_submission.py 직접 실행은 중단 시 2.5시간 전량 재실행)
- **실측 소요**: 2,000문항 = 약 2시간 28분 (4.44초/문항). 마감 6시간 전 착수 권장
- **성적**: LB 최고 0.78459 (참값 0.780±0.005), 2등 0.79542와 약 1.5%p 차

### 미완료 (선택 사항)
- `exp37b-pairwise-fix` — 큐에 남아 있음. 쌍대 비교 가설은 표본 확보 병목으로 **미검증** 상태(기각 아님). 재개 시 자동 실행되나 기대값은 낮음

### 8/30~31 재개 절차
1. `powershell -File aws\start.ps1` → 부팅 자동시작이 systemd 러너 기동 (SSH 불가해도 AWS API로 재부팅 가능)
2. `docs/final-pipeline.md`의 전날 점검 6항목 수행
3. 최종 test 공개(8/31 00:00) 후 1단계 실행 → 2단계 집계 → 3단계 검증 → **사용자 확인 후** 제출

### 그동안 할 일 (GPU 불필요)
- 발표 자료 다듬기 (`docs/presentation.md` — 슬라이드 13장 구조 완성됨)
- 운영진 공지·규칙 변경 모니터링

## 이전 상태 (2026-08-11, 갱신 27) — exp42 완료(Budget Forcing+SC 결합, H20 최종 기각)

- **exp42 완료(Budget Forcing + SC n=8 결합, H20 후속 — 최종 기각)**: `remote/eval_budget_forcing.py --rounds 1 --n 8` — round0(스크립트 자체 기준) 73.9%(357/483) → round1(WAIT 재검토+SC) 74.5%(360/483, +0.6%p, exp32 greedy와 방향 일치). 진짜 비교 기준인 exp05 SC n8(74.7%, 361/483) 대비로는 −0.2%p로 판정 기준(+1%p 이상) 미달 — SC와 결합해도 유의미한 개선 없음. **H20(Budget Forcing) 최종 기각**(greedy 단독·SC 결합 둘 다 실패) — "표 하나의 질을 올리는" 유일한 미탐색 축도 소진, 지금까지 시도한 모든 추론·학습 축이 전부 정체로 확인 완료. 세션 진입 시 이전 세션(자율 러너)이 이미 백그라운드로 실행·완료해둔 결과(01:37→01:55, `eval42_bf_sc.log` 정상 종료, GPU 유휴 확인)를 검증 후 기록만 대행(재실행 없음)
- **큐 다음 항목**: exp43(발표 자산화 — `docs/presentation.md` 갱신) → exp37b(쌍대 비교 재시도, 표본 확보 병목 수정)
- **순위**: 15팀 중, 목표 2등(0.79542) 이내. 갭은 n32w(0.78459) 기준 0.011=9문제로 변화 없음

## 이전 상태 (2026-08-11, 갱신 26) — exp38 완료(제출 다양성 유지), 큐 다음은 exp39(최종 파이프라인 동결 문서)

- **exp38 완료(제출 다양성 유지, 하루 3건 이내 자체 한도)**: exp34에서 생성해둔 미제출 후보 4종(agg/w48/dt32/tb32) 전수 비교 — `submission_agg.csv`(trim25,n32)는 이미 제출된 `submission_tr32.csv`와 **완전 동일**(같은 규칙·같은 덤프, diff=0)이라 중복 제출 회피로 제외, `submission_tb32.csv`(tiebreak_conf,n32)는 이미 제출된 `submission_w32b.csv`와 **5문항만 차이**(15문항 판별 기준 미달)라 제외. 남은 2건만 제출: **w48(weighted,n48) → LB 0.78219**(n32w 0.78459 대비 -0.24%p, 지금까지 제출한 후보 중 n32w 다음으로 높은 점수) / **dt32(drop_trunc,n32) → LB 0.77978**(-0.48%p, w32b·n64w와 동점). 둘 다 새 최고 기록은 아니며 노이즈 범위(exp34b, ±0.48%p) 안 — "집계 규칙이 전부 동등하다"는 기존 결론과 일치. 최종 스택은 여전히 n32w 유지. 오늘 일일 한도 5건 중 4건 사용(exp41의 2건 + 본 실험 2건), 1건 남음
- **큐 다음 항목**: exp39(최종 파이프라인 동결 문서 — `docs/final-pipeline.md`) → exp40(idle guard, 체크 금지) → exp42(BF+SC 결합) → exp43(발표 자산화) → exp37b(쌍대 비교 재시도)
- **순위**: 15팀 중, 목표 2등(0.79542) 이내. 갭은 n32w(0.78459) 기준 0.011=9문제로 변화 없음

## 이전 상태 (2026-08-11, 갱신 25) — exp41 완료(분산 축소 스택, H22 — LB 골드스탠다드 기준 기각), 큐 다음은 exp38(제출 다양성 유지)

- **exp41 완료(분산 축소 스택, H22 신규 — LB 실측으로 최종 기각)**: exp34b(동일 설정 재실행 LB 변동 0.48%p) 후속으로 "시드 3세트(각 n=32)를 96표로 합치면 재실행 변동이 줄어드는가"를 검증. `remote/dump_lb_samples.py --seed`는 이전 세션이 이미 커밋해뒀고, 필요한 5개 시드 덤프(42/43/44/45/46/47)도 세션 진입 시 이미 완료돼 있어 재생성 없이 진행. `remote/merge_lb_dumps.py`(신규)로 두 개의 **독립적인** 96표 스택을 구성 — s3x32(seed42+43+44)와 s3x32v(seed45+46+47, 대조군). **로컬 진단은 표면적 성공**: 두 스택의 예측답 차이가 35문항(4.2%)으로 단일 n=32 재실행 기준(exp34, 67문항/8.06%) 대비 47.8% 감소. 그러나 **둘 다 실제 리더보드에 제출해 비교하니 정반대 결과** — s3x32 LB 0.77617, s3x32v LB 0.78459(=n32w와 정답수 완전 일치), 두 독립 스택 간 LB 점수 차이 **0.842%p(7문항)**가 exp34b의 단일 n=32 재실행 변동(0.481%p, 4문항)보다 오히려 **더 큼**. **핵심 교훈: 로컬 "예측답이 갈리는 문항 수"는 실제 정확도 분산의 신뢰할 수 있는 대리 지표가 아니다** — 몇 문항이 갈리는지보다 그 문항들이 실제로 정답/오답을 얼마나 뒤집는지가 중요한데, 로컬 진단으로는 후자를 알 수 없음(n=1 비교쌍이라 통계적 확증력도 약함). H22(분산 축소) 가설은 CONTEXT.md의 해석 원칙(LB가 골드스탠다드)에 따라 **최종 기각** — 3-시드 결합 스택은 채택하지 않고, 최종 스택은 여전히 n32w(seed42 단일, n=32 가중, 0.78459) 유지. 오늘(2026-08-11) 제출 5건 중 2건 사용(3건 남음)
- **큐 다음 항목**: exp38(제출 다양성 유지, 하루 3건 이내) → exp39(최종 파이프라인 동결 문서) → exp40(idle guard, 체크 금지) → exp42(BF+SC 결합) → exp43(발표 자산화) → exp37b(쌍대 비교 재시도)
- **순위**: 15팀 중, 목표 2등(0.79542) 이내. 갭은 n32w(0.78459) 기준 0.011=9문제로 변화 없음(exp41은 대안 스택 탐색이었을 뿐 최종 스택 자체는 그대로)

## 이전 상태 (2026-08-10 밤, 갱신 24) — exp37 완료(쌍대 비교 토너먼트, 대표 풀이 재현 실패로 미검증·기각), 큐 다음은 exp41(분산 축소 스택)

- **exp37 완료(쌍대 비교 토너먼트, H21 — 성공 기준(+1.5%p) 미달)**: `remote/eval_pairwise_tournament.py`(신규 작성·커밋, margin=3/gen-n=8/votes=5) — val_samples.jsonl(exp31a, n=32)에서 접전(득표차≤3) 65문항을 골라 gen-n=8로 재생성해 후보 답별 대표 풀이(텍스트) 확보 시도했으나 **양쪽 답을 모두 재현한 문항이 0건**이라 토너먼트 판정(A/B 5회 질의)이 한 번도 실행되지 못함. 결과는 baseline(가중 SC n=32, 75.8%/366/483)과 완전 동일(delta +0.0%p) — 성공 기준 미달로 기각. 단, 판정 로직 자체가 안 돌아 **가설(상대비교 우위)은 사실상 미검증**, 원인은 접전 소수표 답을 8샘플 재생성으로 다시 뽑기 어려운 표본 확보 단계 병목으로 추정. results/eval_pairwise_tournament.json, eval37_pairwise.log — 세션 진입 시 이전 세션(자율 러너)이 스크립트 작성·실행·커밋까지 이미 완료해둔 결과를 검증 후 기록만 대행(재실행 없음)
- **큐 다음 항목**: exp41(분산 축소 스택 — 시드 다중 덤프로 재실행 변동 감소 검증) → exp38(제출 다양성 유지, 하루 3건 이내) → exp39(최종 파이프라인 동결 문서) → exp40(idle guard, 체크 금지) → exp42(BF+SC 결합) → exp43(발표 자산화)
- **순위**: 15팀 중, 목표 2등(0.79542) 이내, 갭 0.011=9문제(n32w 0.78459 기준). submission_agg.csv 제출은 다음 가용일 대기 중(일일 한도 소진)

## 이전 상태 (2026-08-10 저녁, 갱신 21) — exp31a/b/c 완료(집계 전략 탐색, n=32 trim_lowconf25%가 +5문제 신호), 큐 다음은 exp35(자율 운영 프로그램)
...
- **exp34 완료(집계 규칙 5종 제출 후보 생성, GPU 불필요)**: exp33 덤프 기반 `remote/make_submission_from_dump.py`를 5회 실행 — w48(가중,n48)/tr32(trim25,n32)/w32b(가중,n32, n32w와 동일설정 재현 대조군)/dt32(drop_trunc,n32)/tb32(tiebreak_conf,n32). 5개 CSV 모두 831행·정수·결측없음 확인, 기존 제출(n32w) 대비 각각 63/76/67/70/67문항 달라 **전부 판별 기준(15문항+) 충족**(판별 불가 후보 없음). **큐 명세대로 제출은 하지 않음** — 5개 후보 중 무엇을 제출할지는 로컬 Claude(계획자)가 판단 필요. 특히 w32b(n32w 재현 대조군)는 둘 다 제출해 LB 점수 차이를 비교하면 "같은 설정의 실행 간 변동폭"을 직접 측정할 수 있음(exp34 큐 명세 의도)
- **큐 다음 항목**: exp32(budget forcing, Wait 재검토로 개별 풀이 질 향상 시도) → exp31a/b/c(집계 전략 대량 탐색) → exp30(최종 리허설) → exp35~40(자율 운영 프로그램)
- **순위**: 15팀 중, 목표 2등(0.79542) 이내, 갭 0.011=9문제(n32w 0.78459 기준, 갱신 대기)
