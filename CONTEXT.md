...
## 현재 상태 (2026-08-07 갱신 4) — exp20 완료(4-shot 프롬프트, 목표 미달·하락), 큐 다음은 exp21(문제 정화)

- **exp20 완료(4-shot 프롬프트, 총력전 카드② 기각)**: `remote/eval_fewshot.py` — 세션 진입 시 이미 이전 세션이 백그라운드로 실행·완료해둔 결과(02:16 시작→02:30 완료, 스크립트는 커밋 b15d974로 이미 반영돼 있었으나 결과 파일 `results/eval_fewshot.json`은 미커밋 상태로 남아 있었음)를 발견해 재실행 없이 검증 후 기록만 수행. `data/sft_short.jsonl`에서 유형별(기하/대수/정수론/응용) 짧은 풀이 4개를 예시로 모든 문제 앞에 고정 삽입 — greedy 70.2%(339/483, zero-shot 69.4% 대비 +0.8%p, 노이즈 범위), **SC n8 73.1%(353/483, zero-shot 74.7% 대비 -1.6%p, 하락)**. 성공 기준(SC≥75.7%) 미달을 넘어 zero-shot보다도 나빠짐 — few-shot 예시가 풀이 형식을 수렴시켜 SC 다수결에 필요한 샘플 다양성을 오히려 줄인 것으로 해석(exp06 QLoRA "자기증류로 다양성 감소" 가설과 유사 메커니즘). 총력전 사이클(exp19~22) 4장 중 2장 소진(H8·퓨샷 모두 기각)
- **큐 다음 항목**: exp21-question-clean (문제 텍스트 정화 — 잡음 패턴 제거 전처리 함수 작성, `remote/clean_question.py`로 분리, 정화 적용/미적용 greedy 비교)
