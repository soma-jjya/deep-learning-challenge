...
## 현재 상태 (2026-08-10 밤, 갱신 22) — exp36 완료(top_p 샘플링 탐색, 목표 미달), 큐 다음은 exp37(쌍대 비교 토너먼트)

- **exp36 완료(샘플링 파라미터 top_p 탐색 — 목표 미달)**: `remote/eval_vllm.py --mode sc --n 16 --top-p 0.95/1.0`(베이스, temp0.7 고정). top_p=0.95 다수결/가중 74.3%(359/483, -1.3%p), top_p=1.0 다수결 74.7%(361/483,-0.9%p)/가중 74.5%(360/483,-1.1%p) — 기준(top_p=0.8, 75.6%) 대비 둘 다 하락, top_p=0.8이 여전히 국소 최적. 성공 기준(유의미한 개선) 미충족이라 덤프·제출 미실행. 세션 진입 시 이미 자율 러너가 두 실행을 백그라운드로 완주해둔 상태(GPU 유휴 확인)를 발견해 재실행 없이 기록만 수행
