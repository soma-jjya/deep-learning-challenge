...
## 현재 상태 (2026-08-06 밤, 갱신) — exp12 완료(GRPO 파일럿, 목표 미달), 큐 다음 항목 exp13(트랙 A 지속 여부 판정)

- **exp12 완료(목표 미달)**: 트랙 A GRPO 파일럿 — `remote/train_grpo.py` 수정 없이 그대로 실행(3000문제, 400스텝, lr5e-6), train_loss 0.02028([wandb ctiym4ny](https://wandb.ai/loonaticvibe2-11-jin-jason/huggingface/runs/ctiym4ny)). 평가: greedy 69.2%(-0.2%p), SC n8 74.5%(-0.2%p, 성공기준 75.7% 미달), 가중투표 74.9%(+0.2%p) — 베이스(69.4/74.7)와 사실상 동일. QLoRA SFT처럼 뚜렷이 하락하진 않았지만(KL 0.003~0.005로 베이스 근처 유지 확인) 보상이 400스텝 내내 0.3~0.65 사이를 노이즈로 진동, 개선 신호 없음. pivot-plan.md 철수 기준("+1%p 미만이면 학습 트랙 동결") 해당 — 다음 exp13에서 지속 여부 최종 판정 필요(로컬 Claude 판단 대기)
- **큐 다음 항목**: exp13-verdict — pivot-plan.md 기준에 따른 트랙 A 지속/철수 판정을 EXPERIMENTS.md에 기록 (실행자 작업 아님, 로컬 Claude 판단 필요)
