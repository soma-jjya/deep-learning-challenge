...
## 현재 상태 (2026-08-06 밤, 갱신) — exp10 완료, 큐 다음 항목 대기

- **exp10 완료**: 확신도 가중 투표(H16) — 일반 다수결 73.7%(356/483) vs 가중 투표 74.5%(360/483), **+0.8%p** 개선(H16 방향 확인, 기대 +1~3%p 하단). 상세는 EXPERIMENTS.md 실험 10 참고. 실행 자체는 이전 세션(사용량 한도로 중단되기 직전)에 이미 완료돼 있었고 로그(`eval10.log`)·결과 파일(`results/eval_base.json`)로 검증 후 기록만 대행(재실행하지 않음) — 이전 기록의 "56초 가짜 완료" 우려와 달리 실제로는 정상 실행됐던 것으로 확인
- **큐 남음**: exp09a(NuminaMath 준비) → exp09b(외부 혼합 QLoRA H12 — 실패 시 정체 3/3 근본 전환)
