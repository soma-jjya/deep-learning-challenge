...
## 현재 상태 (2026-08-10 낮, 갱신 16) — exp29 완료(LB 판별 후보 2종, 둘 다 기존 최고 대비 하락), 큐 다음은 exp33(LB 대량 덤프)

- **exp29 완료(제출 후보 2종, LB 판별)**: `remote/make_submission.py --n 64 --weighted --tag n64w` / `--n 32 --tag n32m` — 두 CSV 모두 831행·정수·결측없음 검증 후 제출. **n64w LB 0.77978**(기존 n32w 0.78459 대비 -0.00481), **n32m LB 0.77737**(-0.00722). 둘 다 기존 최고(n32w, 가중 SC n=32) 대비 하락 — n=32→64 표본 확대는 깨끗한 LB에서도 무의미(H11 정체 재확인), 가중 제거는 LB에서도 손해(H16 가중 투표 유효성 재확인). **최종 스택 여전히 n32w(가중 SC n=32, 로컬 75.8%/LB 0.78459) 확정**
- **운영 사고**: n64w는 이전 세션이 11:24에 이미 제출해둔 것을 확인 안 하고 12:27에 중복 제출 — 일일 제출 한도 1건 낭비. **일일 한도가 실제로 존재함을 이번에 확인**(Kaggle CLI가 매 제출마다 잔여 건수 출력) — 08-10 새벽 기록("일일 한도 없음")은 오류로 정정. 향후 제출 전 이력 확인(`kaggle competitions submissions`) 필수화
- **큐 다음 항목**: exp33-lb-dump (리더보드 표본 대량 덤프, `dump_lb_samples.py --n 96`, ~3시간) → exp34(제출 배치) → exp32(budget forcing) → exp31a/b/c(집계 전략 대량 탐색) → exp30(최종 리허설) → exp35~40(자율 운영 프로그램)
- **순위**: 15팀 중, 목표 2등(0.79542) 이내, 갭 0.011=9문제(n32w 0.78459 기준)
