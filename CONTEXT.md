...
## 현재 상태 (2026-08-06 밤) — 다음 사이클 대기 (인스턴스 정지 상태)

- **큐 등록됨 (미실행)**: exp10(확신도 가중 투표 H16) → exp09a(NuminaMath 준비) → exp09b(외부 혼합 QLoRA H12 — 실패 시 정체 3/3 근본 전환)
- **중단 사유**: 재부팅 후 러너의 exp10이 56초 만에 가짜 완료(커밋 없음) → 서버 Claude **구독 사용량 한도 도달 의심**. 이후 러너 무반응 + 로컬 네트워크에서 SSH 불가로 진단 불가 → 유휴 과금 방지 위해 인스턴스 정지
- **재개 방법**: 몇 시간 뒤(한도 리셋 후) `aws\start.ps1` 또는 start-instances 한 번 — 부팅 자동시작이 최신 코드 pull + 러너 실행, 큐 자동 진행. ntfy(ajudl-rvcmx3ae)로 "실험 러너 시작 → 실험 시작: exp10" 확인
- 오늘 추가된 자가회복 장치: 부팅 자동시작(user-data boothook), 불사 러너(set -e 제거), watchdog 실작업 기준(9f3ac4c — 서버가 아직 pull 안 함, 다음 부팅에 적용), ntfy 웹 API로 SSH 없이 서버 관찰 가능
