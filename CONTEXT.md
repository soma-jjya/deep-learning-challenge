...
## 현재 상태 (2026-08-07 갱신) — exp18b 완료(검증자 학습), 큐 다음은 exp18c(Best-of-N 평가)

- **exp18b 완료(검증자 어댑터 학습, H9)**: `remote/train_verifier.py` — 이 세션 시작 시 이미 백그라운드에서 실행 중이던 학습(이전 세션이 시작, 23:48 시작)을 발견해 중복 실행하지 않고 완료까지 로그를 주기적으로 확인하며 대기(약 81분). r16/alpha32/lr1e-4/ep1, 클래스 균형 1:1(양성 7,760+음성 7,760=15,520, 970스텝), train_loss 0.7613→0.2116(평균 0.2288). [wandb ufarew6n](https://wandb.ai/loonaticvibe2-11-jin-jason/huggingface/runs/ufarew6n). 어댑터 `outputs/verifier/verifier_final` 저장 완료
- **운영 메모**: `remote/run_experiments.sh`를 실행 중인 러너 프로세스(PID 1165, 08-06 17:28부터 실행 중)가 최근 커밋(fe674f0, pgrep 패턴 수정)을 반영하지 못한 채 옛 로직으로 동작 중인 것으로 추정됨 — 이번 세션 진입 시점(23:54)에 학습이 이미 진행 중(23:48 시작)이었음에도 runner.log에 "실험 프로세스 진행 중 — 5분 대기" 로그 없이 exp18b 세션이 연달아 여러 차례 재호출된 흔적이 있었음(중복 실행은 없었음 — 각 세션이 정상적으로 기존 프로세스를 감지하고 대기만 함). 러너 프로세스가 나(이 세션)의 부모 프로세스라 직접 재시작(kill)하면 이 세션 자체가 종료될 위험이 있어 손대지 않음 — **다음 세션이 여유 있을 때 러너를 안전하게 재시작(예: 큐가 빌 때 자연 종료 후 재기동, 또는 별도 세션에서 처리) 권장**, 그 전까지는 세션당 몇 차례 재호출되는 낭비가 계속될 수 있음
- **큐 다음 항목**: exp18c-bestofn-eval (검증자 Best-of-N 평가, `remote/eval_bestofn.py --verifier outputs/verifier/verifier_final --n 8`, 성공 기준 verifier_weighted≥76.5%) → exp18d(성공 시 제출)
