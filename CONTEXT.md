...
## 현재 상태 (2026-08-07 갱신 5) — exp21 완료(문제 정화, 목표 미달·노이즈), 큐 다음은 exp22(적응형 예산, 총력전 마지막 카드)

- **exp21 완료(문제 텍스트 정화, 총력전 카드③ 기각)**: `remote/clean_question.py`(정규식 4종: 번역지시문·이미지링크·Asymptote코드·문두번호 제거) + `remote/eval_question_clean.py` — 세션 진입 시 이미 이전 세션이 백그라운드로 실행·완료해둔 결과(02:37 시작→02:40 완료, 스크립트·결과 파일 모두 미커밋 상태로 남아 있었음)를 발견해 재실행 없이 검증 후 기록만 수행. 검증 483문항 중 정화로 영향받은 문항 25개(5.2%). 전체 greedy: 원문 70.0%(338/483)/정화 69.4%(335/483, -0.6%p). **영향받은 25문항만: 원문 52.0%(13/25)/정화 56.0%(14/25, +4.0%p이나 실제로는 1문항 차이)** — 표본이 25개뿐이라 vLLM 생성 비결정성(±1문항)과 구분 불가, "뚜렷한 개선" 성공 기준 미달. 총력전 사이클(exp19~22) 4장 중 3장 소진(H8·퓨샷·문제정화 모두 기각), 남은 카드는 exp22(적응형 예산)뿐
- **큐 다음 항목**: exp22-adaptive-budget (총력전 마지막 카드 — n8 합의 실패 문제만 추가 24표로 재표결, 성공 기준 ≥76.3%)
