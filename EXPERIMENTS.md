...
| H21 | 쌍대 비교 토너먼트: 접전 문제만 골라 두 후보 풀이를 나란히 제시해 상대 비교(A/B 어느 쪽이 옳은가)로 재판정 — H9(검증자, 절대 채점) 대안 | +1.5%p 이상 | LLM 상대비교>절대평가 통념 | ⏳ **재판정 대기** (exp37 → exp37b 1차) — exp37에서 접전 65문항 전부 대표 풀이 확보 실패(0건)로 토너먼트 미실행. **원인이 표본 부족이 아니라 `eval_pairwise_tournament.py`의 `logprobs=0` 누락 버그(재생성 SamplingParams에서 누락 시 cumulative_logprob이 전부 None → 모든 후보 자동 탈락)로 재규명됨(2026-08-11)** — exp37b 1차(margin≤2, gen-n=32)도 이 버그가 있는 채로 실행돼 동일하게 0건이 나왔으므로 그 결과 역시 **무효**. 버그 수정(logprobs=0 추가) 완료, 정정판 재시도(margin=3, gen-n=32)가 큐에 등록됨 — 가설 자체는 아직 한 번도 실제로 검증된 적 없음 |
