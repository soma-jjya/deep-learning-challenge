...
| H21 | 쌍대 비교 토너먼트: 접전 문제만 골라 두 후보 풀이를 나란히 제시해 상대 비교(A/B 어느 쪽이 옳은가)로 재판정 — H9(검증자, 절대 채점) 대안 | +1.5%p 이상 | LLM 상대비교>절대평가 통념 | ❌ **최종 기각** (exp37, exp37b) — exp37(margin≤3, gen-n=8): 접전 65문항 전부 재현 실패(확보 0건). exp37b(margin≤2로 좁히고 gen-n=32로 4배 증량해 재시도): 접전 50문항으로 줄었음에도 **여전히 확보 0건**, 토너먼트 0문항 실행, baseline과 완전 동일(366/483, +0문제). 재생성 샘플 수를 늘려도 소수 득표 답 재현율이 개선되지 않음을 확인 — 가설(상대비교 우위) 자체가 표본 확보 단계를 넘어 검증된 적이 없는 채로 트랙 종료 |
...
## 실험 37b: 쌍대 비교 토너먼트 재시도 — 표본 확보 병목 재현, H21 최종 기각 (2026-08-11, AWS)

- **배경**: exp37이 접전 65문항 전부에서 대표 풀이(양쪽 후보 답) 확보에 실패(gen-n=8이 원인으로 추정)해 토너먼트 판정 로직이 한 번도 실행되지 못했다. 큐 명세대로 병목을 수정해 재시도: ① 접전 기준을 margin≤3→**margin≤2**로 좁혀 진짜 접전만 선별 ② 대표 풀이 확보용 재생성을 gen-n=8→**gen-n=32**(4배)로 늘려 소수 득표 답의 재현 확률을 높임 ③ 나머지(votes=5, 쌍대 비교 로직)는 exp37과 동일
- **설정**: `remote/eval_pairwise_tournament.py --margin 2 --gen-n 32 --votes 5` (코드 수정 없이 기존 스크립트의 CLI 인자만 변경 — 스크립트가 이미 margin/gen-n을 파라미터화해뒀음)
- **결과**: margin≤2로 좁히자 접전 문항이 65→**50개**로 감소. 그러나 gen-n을 4배(8→32, 문항당 재생성 1600개)로 늘렸음에도 **대표 풀이 양쪽 확보 0건**(exp37과 동일하게 전량 실패) → 토너먼트 0문항 실행, 전량 baseline(가중 다수결 n=32) 값으로 폴백. `[baseline weighted n=32] 75.8% (366/483)` → `[pairwise tournament] 75.8% (366/483), delta=+0문제 (+0.00%p)`. 성공 기준(+5문제 이상) 명백히 미달
- **판정**: **기각**. exp37의 가설("재생성 샘플 부족이 원인")이 4배 증량으로도 개선되지 않아 반증됨 — 소수 득표 답은 원래 표본(val_samples.jsonl, exp31a)에도 극히 드물게만 등장하므로, 온도 0.7 재생성으로는 gen-n을 얼마나 늘려도 그 특정 답을 다시 뽑을 확률 자체가 낮은 것으로 추정(구조적 병목이지 표본 수 문제가 아님). 이 방식으로 대표 풀이를 확보하려면 재생성이 아니라 **원본 n=32/64 덤프에서 애초에 풀이 텍스트까지 저장**하는 재설계가 필요하나, 큐에 없는 추가 실험이라 임의로 진행하지 않음. H21(쌍대 비교 토너먼트) **최종 기각**
- **결과 파일**: `results/eval_pairwise_tournament.json`(exp37 결과에서 덮어씀), `eval37b_pairwise.log`
- **사고 기록**: 세션 진입 시 이전 세션(자율 러너)이 이미 스크립트 실행·결과 커밋(03:00 정상 종료, GPU 유휴 확인)까지 완료해둔 상태를 발견 — 재실행 없이 로그·GPU 상태·결과 파일 일치 확인 후 기록만 대행
- **다음**: 큐 완전 소진 — 로컬 Claude(계획자)가 다음 가설 등록 필요
