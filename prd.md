...
- **✅ 운영진 답변 확정 (2026-08-07)**: ① 같은 베이스의 **멀티 LoRA 어댑터 앙상블 허용** ② **검증자(verifier) 어댑터로 Best-of-N 선별 허용** (외부 모델 사용 아님). 단서: 유형별 전문 어댑터 다수 라우팅 등 "사실상 복수 특화 모델" 수준은 취지 위반 소지 — **합리적 범위**로 사용 (우리 구성: 검증자 어댑터 1개 + 베이스 생성 = 안전). 추후 어댑터 수 상한·문제당 생성 횟수 제한이 공지될 수 있음
