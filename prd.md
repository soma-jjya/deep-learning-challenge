...
- **⚠️ 추론 시 코드 실행·도구 호출 금지 (운영진 Q&A 확정, 2026-08)**: TIR/Program-of-Thought(모델이 쓴 파이썬 실행), 사전 구현한 계산 유틸 호출 모두 불허. "모델의 추론 출력만으로" 답 도출이 원칙
- **질의 중 (답변 대기)**: ① 같은 베이스의 LoRA 어댑터 여러 개를 추론 시 앙상블(다수결) ② 같은 베이스로 학습한 검증자(verifier) 어댑터로 Best-of-N 선별 — 허용 여부 회신 오면 여기 갱신

### 데이터셋 검수 반영 (운영진 공지 2026-08-03)
- **리더보드**: 오류 169문항 제외 → `deep_chal_math_leaderboard_filtered.csv` (831문항)만 사용. 기존 leaderboard.csv 폐기. 공지 이후 제출분부터 새 기준 채점 — **기존 제출물은 재제출해야 새 기준 점수를 받음**
- **train**: 오류 627문항 id 목록 = `train_filtered_ids.csv` (레포에 보관). 학습·검증에서 제외할 것
- 오류 기준: 이미지/본문 누락, 지시문 오염, 라벨 불일치, 정수 미확정
