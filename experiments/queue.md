...
- [ ] exp06c-qlora-gentle: H3 재시도 (lr 5e-5, ep1, 최단풀이). **학습은 이미 완료됨** — 어댑터: `outputs/qlora_gentle/qlora_r16_lr5e-05_ep1_final`, train loss 0.2429, wandb sv8lkhho. **재학습 금지.** 남은 것: `remote/eval_vllm.py --mode both --adapter outputs/qlora_gentle/qlora_r16_lr5e-05_ep1_final` 평가가 로컬 Claude에 의해 시동됐을 수 있음 — eval06c.log 또는 results/eval_*qlora_gentle*.json이 있으면 그 결과를 기록하고, 없으면 위 명령 실행 후 기록. exp05(69.4/74.7)와 비교해 verdict 판정
