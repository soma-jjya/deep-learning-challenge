...
- [ ] exp10-weighted-vote: 확신도 가중 투표 (H16). `remote/eval_vllm.py --mode sc --n 8` 실행 — 스크립트가 개선되어 일반 다수결과 가중 투표를 한 번에 측정한다. 출력의 `[sc_n8]`와 `[sc_n8+weighted]` 두 수치를 기록하고 exp05(74.7%)와 비교. 베이스 모델(어댑터 없음)
- [ ] exp09a-numina-prep: 외부 데이터 준비 (H12). `uv run python remote/prep_numina.py --take 30000` 실행 (datasets 라이브러리 필요 — 설치돼 있음). 채택 개수와 혼합 결과(외부+자체) 통계를 기록. GPU 불필요, 다운로드 수 GB
