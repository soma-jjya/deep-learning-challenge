...
- [ ] exp12-grpo-pilot: 트랙 A — GRPO 강화학습 파일럿 (H17). `nohup uv run python remote/train_grpo.py > train_grpo.log 2>&1 &`로 학습 (trl GRPOTrainer + unsloth, 3천 문제, 400스텝, 수 시간). 라이브러리 API 불일치 에러가 나면 스크립트를 최소 수정해 실행하고 수정 내용을 커밋 메시지에 명시. 완료 후 `remote/eval_vllm.py --mode both --adapter outputs/grpo_pilot/grpo_pilot_final` 평가. **성공 기준: SC n8 ≥ 75.7%** (베이스 +1%p). wandb 링크·보상 추이도 기록
- [ ] exp13-verdict: pivot-plan.md의 성공/철수 기준에 따라 트랙 A 지속 여부 판정을 EXPERIMENTS.md에 한 줄 기록
