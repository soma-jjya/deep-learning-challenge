...
끝나면: ① 결과 수치를 EXPERIMENTS.md 표와 해당 실험 섹션에 기록 ② experiments/log.csv에 한 행 추가 (기존 헤더 스키마 그대로, 실패·에러도 verdict=failed로 기록) ③ queue.md에서 해당 항목을 [x]로 바꾸고 결과 한 줄 덧붙임 ④ git add -A && git commit && git push
