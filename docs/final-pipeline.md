# 최종 test 실행 절차 (2026-08-31) — 확정본

> exp30 리허설(2026-08-10, 831문항 실측)로 검증된 절차. **당일에는 이 문서만 따른다.**
> 리허설 원본 수치는 `results/rehearsal_report.md`.

## 확정 스택

**Qwen2.5-3B-Instruct (무학습 베이스) + 확신도 가중 Self-Consistency n=32, temperature 0.7, top_p 0.8, seed=42**

- 어댑터 미사용 — 학습 **6전**(SFT×3, GRPO×2, DPO×1) 전부 성공 기준 미달로 확인돼 최종 스택에서 제외 (exp48: DPO는 시드 대응 비교에서 부호가 섞여 효과 없음 확정)
- 근거: 로컬 검증 75.8%, 리더보드 0.78459 (참값은 재실행 변동 감안 시 0.780±0.005 — 아래 "선정 근거" 참고)

## LB 점수가 확인된 전체 제출 이력

| exp | 날짜 | 설정 | LB | 비고 |
|---|---|---|---|---|
| exp01 | 07-31 | greedy, boxed추출 버그 상태 | 0.648 | 구채점기준(1000문항), 신기준(831)과 직접 비교 불가 |
| exp06b | 08-06 | 베이스, SC n=8 가중 | 0.77015 | 로컬 74.7%와 정합, 최초 신기준 제출 |
| **exp26** | 08-07 | 베이스, **SC n=32 가중 seed42** | **0.78459** | 역대 최고점. 현재 확정 스택과 동일 설정 |
| exp29-1 | 08-10 | 베이스, SC n=64 가중 | 0.77978 | n32→64 확대 무의미(H11 LB 재확인), n32w 대비 −0.48%p |
| exp29-2 | 08-10 | 베이스, SC n=32 단순다수결(가중無) | 0.77737 | 가중 제거 시 −0.72%p, 가중 투표 유효성 재확인 |
| exp34b | 08-10 | **exp26과 완전 동일 설정, 표본만 재추출** | 0.77978 | **동일 설정 재실행만으로 0.48%p 변동** — 노이즈 하한 측정치 |
| exp41a | 08-11 | 3-시드(42/43/44) n32 병합 96표 가중 | 0.77617 | 분산축소 가설(H22) 실험군 |
| exp41b | 08-11 | 3-시드(45/46/47) n32 병합 96표 가중 | 0.78459 | H22 대조군 — exp41a와 0.84%p 차이(노이즈 하한보다 큼) → H22 기각 |
| exp38-w48 | 08-11 | 베이스, SC n=48 가중 | 0.78219 | n32w 대비 −0.24%p, 노이즈 범위 내 |
| exp38-dt32 | 08-11 | 베이스, SC n=32 + 잘린풀이 제외 | 0.77978 | 가중 투표와 사실상 동률 |

### 선정 근거 (LB 최고점이 아니라 원리로 선택)

- 단순 최고점 기준이면 exp26/exp41b(0.78459)이지만, **exp34b가 "완전히 동일한 설정"을 표본만 다시 뽑아 재실행한 결과 0.77978로 0.48%p 하락**했다 — 즉 0.78459는 참값이 아니라 노이즈 상한에 가까운 관측치. 하나의 관측 최고점을 쫓는 것은 과적합이다.
- 대신 **"가중 SC n=32, temp0.7/top_p0.8, 어댑터 없음"이라는 하나의 설정이 4번 관측**됐다(exp26 0.78459, exp34b 0.77978, exp41b 0.78459[동치], exp38-dt32 0.77978[근사 변형]) — 평균 0.780 근방으로 수렴하며, 이 값이 다른 축(n=64 확대, 가중 제거, 다중시드 병합)보다 항상 같거나 높다. **로컬 검증(75.8%)과도 정합**(오프셋 +2.7%p 일관).
- n=48(0.78219)·n=64(0.77978)·단순다수결(0.77737)·3시드병합(0.77617/0.78459) 등 다른 모든 변형은 n=32 가중 대비 개선이 없거나 노이즈 범위 안에 머물렀다 → **탐색한 축 중 "SC n=32 가중"이 유일하게 반복 검증된 안정적 선택지**.

## ⚠️ 반드시 dump 방식으로 실행할 것

`make_submission.py`를 직접 쓰면 **중단 시 처음부터 재실행**(최대 2.5시간 손실)이다. 리허설에서 확인된 최대 리스크.
아래 2단계 방식은 100문항마다 저장돼 **어디서 죽어도 이어서 재개**된다.

### 1단계 — 표본 생성 (약 2시간 30분, 재개 가능)

```bash
cd ~/work/deep-learning-challenge && git pull
nohup uv run python remote/dump_lb_samples.py \
  --n 32 --temp 0.7 --top-p 0.8 --seed 42 \
  --lb-csv deep-learning-challenge-2026/<최종test파일>.csv \
  --out results/final_samples.jsonl > dump_final.log 2>&1 &
```

- 진행 확인: `tail -f dump_final.log` → "진행 N/<전체>문항"
- **중단됐다면 같은 명령을 그대로 재실행** (완료분은 건너뜀).
- **(2026-08-19 추가) 파라미터가 다르면 스크립트가 재개를 거부한다.** `.progress`에 `--n/--seed/
  --temp/--top-p/--max-tokens/--adapter/test파일명`의 지문이 저장되고, 재실행 시 대조한다.
  이전에는 명령을 다시 타이핑하다 한 글자만 달라져도 **앞뒤 설정이 다른 덤프가 조용히 만들어졌다.**
- 시간이 부족해 `--n 32 → 16`으로 낮추는 것은 **정당한 예외**다. 이때만 `--force-resume`을 붙인다
  (앞부분에 표본이 더 많을 뿐이고, 2단계에서 `--n 16`으로 자르면 전 문항이 같아진다).
- 청크 도중에 죽어 `.progress`와 jsonl이 어긋나도 **스크립트가 jsonl을 읽어 자동 보정**한다
  (같은 문항이 두 번 기록되지 않는다).

### 2단계 — 제출 파일 생성 (수 초, GPU 불필요)

```bash
uv run python remote/make_submission_from_dump.py \
  --dump results/final_samples.jsonl --rule weighted --n 32 \
  --lb-csv deep-learning-challenge-2026/<최종test파일>.csv --tag final
```

### 3단계 — 제출 전 검증 (필수) — **이제 코드로 한다**

```bash
uv run python remote/validate_submission.py   --sub results/submission_final.csv   --test deep-learning-challenge-2026/<최종test파일>.csv   --dump results/final_samples.jsonl --n 32
```

**종료 코드 0이 아니면 절대 제출하지 않는다.** 이 검증기는 아래를 전부 자동으로 본다 —
헤더가 소문자 `id,answer`인지 / 행 수와 **id 집합이 test와 정확히 일치**하는지 / 중복·결측 /
answer가 전부 **int64 범위 안의 정수**인지 / 0인 답 비율 / 덤프 커버리지·문항당 표본 수 /
추출 실패율·잘림 비율.

임계값은 추측이 아니라 우리 제출 25건 실측으로 잡았다 — **정상 24건의 0 비율은 2.05~3.25%**,
**2026-08-15 사고 파일은 97.71%**였다. 실제 0.78459 제출 파일로 통과를, 사고 파일로 차단을
각각 확인했고, 병리 입력 17종에 대한 dry-run(`remote/dryrun_submission_path.py`)도 통과한다.

아래는 검증기가 보는 것을 사람이 이해하기 위한 설명이며, **손으로 다시 확인할 필요는 없다.**

#### (참고) 검증 항목의 배경

- **덤프 커버리지 확인이 먼저다.** `make_submission_from_dump.py`는 덤프에 없는 문항을 **조용히 0으로**
  채운다. 2026-08-15 스모크 테스트에서 20문항만 덤프했더니 **831행 중 811개가 0인 파일**이 나왔고,
  행수·결측·중복 검사를 **전부 통과**했다. 즉 아래 항목만으로는 이 사고를 잡지 못한다.
  → 지금은 스크립트가 기본적으로 **거부**하도록 고쳤다(`--allow-partial` 없이는 진행 불가).
  덤프가 중간에 죽었다면 **같은 명령을 재실행**해 끝까지 채운 뒤 2단계로 갈 것.
  ```bash
  # 덤프 줄 수 == test 문항 수 인지 눈으로도 확인
  wc -l < results/final_samples.jsonl
  wc -l < deep-learning-challenge-2026/<최종test파일>.csv   # 헤더 1줄 감안
  ```
- 행수 = **test 파일의 실제 문항 수** / 컬럼 `id`,`answer` / answer 전부 정수 / 결측·중복 0건
- **answer가 0인 문항 수 확인** — 정상 실행에서는 극소수여야 한다. 수십·수백 개면 덤프 누락이다
- **`id`는 소문자** (대회 문서엔 `ID`로 적혀 있으나 채점기는 소문자 — 2026-07-31 첫 제출 ERROR로 확인된 사항)

### 4단계 — 제출 (✅ 2026-08-19 운영진 답변으로 확정: **구글 폼**, kaggle CLI 아님)

- **창구**: 8/31 00:00 test 공개 → **같은 날 23:59까지 구글 폼으로 제출** (운영진 공식 답변)
- **재제출(수정) 가능** — 마감 전까지 여러 차례 허용 (운영진 공식 답변)
- **전략**: 검증기 통과 즉시 **1차 제출로 "제출물 존재"부터 확보**한다. 결함이 발견되면 수정
  재제출. 마감 직전 단일 제출 도박 금지.
- 폼 링크·추가 기입 항목(팀명 등)은 당일 공지 확인 — 전날 점검 항목에 포함
- kaggle CLI 제출은 **리더보드(8/30까지)** 에만 유효하다

**⚠️ 최종 제출(폼 전송)은 사용자 확인 후에만. 자동 제출 금지.**

## 시간 계획

> ✅ **2026-08-19 운영진 답변으로 확정: 최종 test는 약 2,000문항.** n=32로 약 2시간 28분 —
> 아래 표의 2,000 행이 기준이다. 당일 실제 행 수를 세는 절차는 방어적으로 유지한다.

**당일 첫 번째로 할 일** — 파일을 열자마자 문항 수를 세고 소요를 계산한다:

```bash
python -c "import csv; print('문항 수:', sum(1 for _ in csv.DictReader(open('deep-learning-challenge-2026/<최종test파일>.csv',encoding='utf-8'))))"
```

| test 문항 수 | n=32 생성 시간 | n=16 생성 시간 |
|---|---|---|
| 2,000 | 2시간 28분 | 1시간 14분 |
| 2,500 | 3시간 05분 | 1시간 33분 |
| 3,000 | 3시간 42분 | 1시간 51분 |
| 3,500 | 4시간 19분 | 2시간 10분 |

계산식: `시간 = 문항수 × 4.44초 ÷ 3600` (n=32 실측치, exp30 리허설 831문항 기준)

**시간이 빠듯하면 주저 말고 `--n 16`으로 내릴 것.** 로컬 검증에서 n=16은 75.6%, n=32는 75.8%로
차이가 0.2%p이며 이는 측정 노이즈(±1%p) 한참 안쪽이다 — **생성 시간을 절반으로 줄이는 대가로
잃는 것이 사실상 없다.** 제출 자체를 못 하는 것이 유일한 실패이므로 이 교환은 언제나 유리하다.

| 구간 | 소요 | 비고 |
|---|---|---|
| 표본 생성 | 위 표 참조 | 4.44초/문항 (n=32) |
| 집계·검증 | 5분 | GPU 불필요 |
| **중단 1회 대비 여유** | 생성 시간만큼 | 재개되므로 실제로는 부분 손실만 |
| **권장 착수 시각** | **마감 (생성시간 × 2 + 1시간) 전** | 2,000문항이면 6시간, 3,500문항이면 9시간 30분 |

## 사전 점검 (전날)

- [ ] **최종일 경로 재확인** — ✅ 제출=구글 폼·재제출 가능·약 2,000문항은 운영진 답변으로
  확정됨(2026-08-19). 남은 확인: ① test 파일이 정확히 어디에 올라오는지 ② **구글 폼 링크**가
  어디에 공지되는지 ③ 폼의 요구 항목(파일 형식·팀명 등)
- [ ] **`uv run python remote/dryrun_submission_path.py` 실행 → 17/17 통과 확인** (GPU·vLLM 불필요, 수 초)
- [ ] `powershell -File aws\start.ps1`로 인스턴스 기동 → **SSH 안 되면 `aws\ensure-ssh.ps1` 먼저** (IP 로테이션) → GPU 인식 확인 (`nvidia-smi`)
- [ ] `git pull`로 최신 스크립트 확보
- [ ] 최종 test CSV를 `deep-learning-challenge-2026/`에 배치, 컬럼명(`id`,`question`) 확인
- [ ] 디스크 여유 1GB 이상 (덤프 jsonl + 로그)
- [ ] 구글 계정 로그인 상태 확인 (최종 제출이 구글 폼이므로)

## 8/31 당일 체크리스트

- [ ] ⚠️ **test 문항의 질문 텍스트를 Claude 세션·화면에 출력하지 않는다** (5.3.b/c의 보수적 준수 —
  Claude도 상용 API다). 파일 검사는 문항 수·컬럼명·id만 출력하는 스크립트로 한다:
  문항 수 세기(아래 명령), `validate_submission.py`(id만 표시), dump 진행 로그(개수만).
  `head`/`cat`으로 test CSV 내용을 직접 열지 말 것.
- [ ] **가장 먼저 test 문항 수를 세고** 위 표로 소요·착수 시각을 재계산 (2,000 가정을 그대로 믿지 말 것)
- [ ] 계산된 착수 시각에 시작 — 여유가 없으면 `--n 16`으로 즉시 전환(0.2%p 손실, 시간 절반)
- [ ] 1단계 명령 실행 후 `dump_final.log`에서 "진행 N/<전체>문항" 로그가 정상적으로 늘어나는지 5분 내 확인
- [ ] 표본 생성 완료(전체/전체) 후 2단계 실행, `results/submission_final.csv` 생성 확인
- [ ] **`remote/validate_submission.py` 실행 → 종료 코드 0 확인** (실패면 제출 금지)
- [ ] **제출 전 사용자에게 파일 경로·검증 결과 보고하고 명시적 확인 받기** — 자동 제출 절대 금지
- [ ] 구글 폼 제출 후 **접수 확인 화면·메일을 캡처 보관** — 재제출했다면 최종본 접수 여부 재확인
- [ ] 폼 재제출은 **결함 수정 시에만** — 같은 스택 재생성·재제출은 노이즈만 추가(exp34b). 제출 이력·최종 점수를 EXPERIMENTS.md에 기록

## 사고 대응

| 상황 | 대응 |
|---|---|
| 생성 프로세스 사망 | 같은 명령 재실행 → 완료분 건너뛰고 재개 |
| **인스턴스 시작 실패 (용량 부족)** | ① 몇 분 간격 재시도 ② 안 되면 다른 가용영역에 `aws\launch.ps1`로 새 인스턴스 + `bootstrap.sh` 재구축 (모델 캐시 재다운로드 ~10분, git clone 포함 ~20분) ③ 최후: Kaggle T4×2 노트북 — 속도 2~3배 느리므로 이 경로 확정 즉시 착수하고 `--n 16` |
| 러너/서버 무응답 | AWS API로 재부팅 (SSH 불가해도 가능) → systemd가 러너 자동 기동 |
| **SSH 접속 timeout** | **먼저 `powershell -File aws\ensure-ssh.ps1`을 실행할 것.** 2026-08-11에 원인이 규명됐다 — 네트워크 차단이 아니라 **ISP의 공인 IP 로테이션**으로 보안그룹 허용에서 이탈하는 것이었다(한 세션 안에서 211.235.66.89 → 58.231.140.230으로 변경). 이 스크립트가 현재 IP를 멱등 재등록한다. 그래도 안 되면 AWS API·ntfy는 별개로 동작하므로 재부팅 경로로 우회 |
| GPU 메모리 부족 | `--chunk 50`으로 청크 축소 (리허설 최대 95% 사용, 여유 적음) |
| 시간 부족 | `--n 16`으로 축소 (로컬 75.6% — n32와 0.2%p 차이, 노이즈 안쪽). 망설이지 말 것 — 미제출이 유일한 실패다. **재개 중 낮추는 경우 `--force-resume` 필요** |
| 재개가 거부됨 | 파라미터 지문 불일치다. 에러가 **어느 값이 달라졌는지 그대로 출력**하므로 그것부터 볼 것. 의도한 변경이면 `--force-resume`, 아니면 원래 값으로 재실행 |
| 제출 검증 실패 | 종료 코드가 0이 아니면 **제출하지 말 것.** 메시지가 원인을 지목한다 — 0이 많으면 덤프 누락, int64 초과면 잘린 풀이에서 나온 거대 정수 |
| 거대한 정수 답 | 2단계가 자동으로 **범위 안 후보로 재집계**하고 그 문항을 로그에 남긴다. 조치 불필요 |
